...
class Model_policies extends CI_Model
{
	...
	{		
		...
		{	$param		= $this->nsession->userdata('POLOCIES_MASTER');
		...
		$this->nsession->set_userdata('POLOCIES_MASTER', $sessionDataArray);		
		
		$start 	= 0;
		$where	= ' WHERE 1 ';
		
		if($search_keyword != ''){
			$where.= " AND policies_name like '%".$search_keyword."%' ";
		}
		
		$sql=" SELECT COUNT(*) as TotalrecordCount FROM ".POLICY_MASTER." ".$where." ";
        ...
        public function updateStatus($tableName, $idArr, $updateArr)
	{
		...
		$sql 	= "SELECT policies_name FROM ".POLICY_MASTER." WHERE `policies_master_id` = '".$idArr['policies_master_id']."'";
		$rs 	= $this->db->query($sql);
		$rec 	= $rs->result_array();
