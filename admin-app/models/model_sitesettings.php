...
class Model_sitesettings extends CI_Model
{
	...
	public function getListEmails(&$config,&$start,&$tabId){
		...
		elseif($tabId=='social')
		{
			$where	= " WHERE 1 AND status = 'active' AND sitesettings_id IN(9,10,11,15,16,18,19)";
		}
		elseif($tabId=='site')
		{
			$where	= " WHERE 1 AND status = 'active' AND sitesettings_id IN(7,8,20,21,27,22)";
		}
		//elseif($tabId=='email')
		//{
		...
		{
		  $where	= " WHERE 1 AND status = 'active' AND sitesettings_id IN(1,13,17)";
		}
