...
class Model_property_type extends CI_Model
{
	...
	{		
		...
		{	$param		= $this->nsession->userdata('PROPERTY_TYPE_SEARCH');
		...
		$this->nsession->set_userdata('PROPERTY_TYPE_SEARCH', $sessionDataArray);		
		
		$start 	= 0;
		$where	= ' WHERE 1 ';
		
		if($search_keyword != ''){
			$where.= " AND property_type_name like '%".$search_keyword."%' ";
		}
		
		$sql=" SELECT COUNT(*) as TotalrecordCount FROM ".PROPERTY_TYPE." ".$where." ";
        ...
        public function updateStatus($tableName, $idArr, $updateArr)
	{
		...
		$sql 	= "SELECT property_type_name FROM ".PROPERTY_TYPE." WHERE `property_type_id` = '".$idArr['property_type_id']."'";
		$rs 	= $this->db->query($sql);
		$rec 	= $rs->result_array();
