...
class Model_province extends CI_Model
{
	...
	{		
		...
		{	$param		= $this->nsession->userdata('PROVINCE_MASTER_SEARCH');
		...
		$this->nsession->set_userdata('PROVINCE_MASTER_SEARCH', $sessionDataArray);		
		
		$start 	= 0;
		$where	= ' WHERE 1 ';
		
		if($search_keyword != ''){
			$where.= " AND province_name like '%".$search_keyword."%' ";
		}
		
		$sql=" SELECT COUNT(*) as TotalrecordCount FROM ".PROVINCE_MASTER." ".$where." ";
	...
	public function updateStatus($tableName, $idArr, $updateArr)
	{
		...
		$sql 	= "SELECT province_name FROM ".PROVINCE_MASTER." WHERE `province_id` = '".$idArr['province_id']."'";
		$rs 	= $this->db->query($sql);
		$rec 	= $rs->result_array();
