...
class Model_discountcode extends CI_Model
{
	...
	{		
		...
		{	$param		= $this->nsession->userdata('DISCOUNTCODE_MASTER_SEARCH');
		...
		$this->nsession->set_userdata('DISCOUNTCODE_MASTER_SEARCH', $sessionDataArray);		
		
		$start 	= 0;
		$where	= ' WHERE 1 ';
		
		if($search_keyword != ''){
			$where.= " AND code like '%".$search_keyword."%' or status like '%".$search_keyword."%'";
		}
		
		$sql=" SELECT COUNT(*) as TotalrecordCount FROM ".DISCOUNTCODE_MASTER." ".$where." ";
		...
		$config['start'] = $start;
		$sql = "SELECT *
			FROM ".DISCOUNTCODE_MASTER.$where."
