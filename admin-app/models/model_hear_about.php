...
class Model_hear_about extends CI_Model
{
	...
	{		
		...
		{	$param		= $this->nsession->userdata('HEAR_ABOUT_SEARCH');
		...
		$this->nsession->set_userdata('HEAR_ABOUT_SEARCH', $sessionDataArray);		
		
		$start 	= 0;
		$where	= ' WHERE 1 ';
		
		if($search_keyword != ''){
			$where.= " AND hear_about_name like '%".$search_keyword."%' ";
		}
		
		$sql=" SELECT COUNT(*) as TotalrecordCount FROM ".HEAR_ABOUT." ".$where." ";
        ...
        public function updateStatus($tableName, $idArr, $updateArr)
	{
		...
		$sql 	= "SELECT hear_about_name FROM ".HEAR_ABOUT." WHERE `hear_about_id` = '".$idArr['hear_about_id']."'";
		$rs 	= $this->db->query($sql);
		$rec 	= $rs->result_array();
