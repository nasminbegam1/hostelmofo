...
class Model_language extends CI_Model
{
	...
	{		
		...
		{	$param		= $this->nsession->userdata('LANGUAGE_SEARCH');
		...
		$this->nsession->set_userdata('LANGUAGE_SEARCH', $sessionDataArray);		
		
		$start 	= 0;
		$where	= ' WHERE 1 ';
		
		if($search_keyword != ''){
			$where.= " AND property_language_name like '%".$search_keyword."%' ";
		}
		
		$sql=" SELECT COUNT(*) as TotalrecordCount FROM ".LANGUAGE_MASTER." ".$where." ";
        ...
        public function updateStatus($tableName, $idArr, $updateArr)
	{
		...
		$sql 	= "SELECT property_language_name FROM ".LANGUAGE_MASTER." WHERE `property_language_id` = '".$idArr['property_language_id']."'";
		$rs 	= $this->db->query($sql);
		$rec 	= $rs->result_array();
