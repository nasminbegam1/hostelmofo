    ...
    <div class="col-sm-12">
      <div class="panel panel-default">
        ...
        <div class="panel_controls">
          <form name="perPageFrm" id="perPageFrm" method="post" action="<?php echo BACKEND_URL;?>property/">
            <div class="row">
              <div class="col-sm-4">
  ...
  <div class="row">
    <div class="col-sm-12">
      <div class="panel panel-default">
		...
		<th data-toggle="true">Record Type</th>
                <th data-hide="phone,tablet">Optional Title</th>
              ...
              <tr>
                <td colspan="9"><div class="itemActions">
                    ...
                    <select class="styled" name="apply_action" id="apply_action" >
                      ...
                      <option value="Both">Sales And Rental Property</option>		      
                    </select>
                  </div>
                  <?php
			...
			for($i=0; $i<count($propertyMasterList); $i++){ //pr($propertyMasterList); 
				...
				?>
					  <tr <?php echo $class; ?>>
						...
						<td>
						<form name="frmChangRecordtype" id="frmChangRecordtype_<?php echo stripslashes($propertyMasterList[$i]['record_type']);?>" method="post" action="<?php echo FRONTEND_URL.'property/change_record_type/'.$propertyMasterList[$i]['property_id'].'/'.$page; ?>">
						<select name="selChangeType" onchange="changeRecordType('<?php echo stripslashes($propertyMasterList[$i]['property_id']);?>', this.value);">
							<option value="Rental" <?php if($propertyMasterList[$i]['record_type'] == "Rental") echo 'selected="selection"'; ?>>Rental</option>
							<option value="Sales" <?php if($propertyMasterList[$i]['record_type'] == "Sales") echo 'selected="selection"'; ?>>Sales</option>
							<option value="Both" <?php if($propertyMasterList[$i]['record_type'] == "Both") echo 'selected="selection"'; ?>>Both</option>
						</select>
						<input type="hidden" name="property_id" value="<?php echo $propertyMasterList[$i]['property_id'];?>" />
						</form>						
						</td>
...
<script type="text/javascript">
	function changeRecordType(property_id, propType){
		//alert(property_id+ propType);
		$("frmChangRecordtype_"+property_id).submit();		
		$.ajax({
		type: "POST",
		dataType: "HTML",
		url: "<?php echo BACKEND_URL; ?>" + "property/change_record_type/"+property_id+"/",
		data: { selChangeType: propType},
