...
class Faq extends CI_Controller
{
    ...
    function is_faq_title_exists($faq_title)
	{
		...
		$bool = $this->model_basic->checkRowExists($this->faqMaster, $whereArr);
		if($bool == 0)
		{
    ...
    public function add_faq_master()
    {
		...
		if($this->input->get_post('action') == 'Process')
		{
			...
			} else
			{
				...
				$ret   = $this->model_basic->insertIntoTable($this->faqMaster,$insertArr);
				if($ret)
				{
    	...
    	public function edit_faq_master()
	{
		...
		if($this->input->get_post('action') == 'Process')
		{
			...
			else
			{
				...
				$ret   = $this->model_basic->updateIntoTable($this->faqMaster,$idArr, $updateArr);
		...
		$rs = $this->model_basic->getValues_conditions($this->faqMaster, '', '', $Condition);
    ...
    function delete_faq_master()
	{
		...
		$where			= "faq_id = '".$faq_id."'";
			...
			$this->model_basic->deleteData($this->faqMaster, $where);
	...
	private function faqbatchstatus($status, $idArray)
	{
		...
		if($status == '')
			return false;
		$return 	= $this->model_basic->changeStatus($this->faqMaster, $idArray, 'faq_status', $status, 'faq_id');
