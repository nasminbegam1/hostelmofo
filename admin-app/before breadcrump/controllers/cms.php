			...
			if ($this->form_validation->run() != FALSE && $action != FALSE)
			{
				...
				if ($_FILES['cms_image']['name'] != "")
				{
					...
					else
					{
						...
						$ret   = $this->model_basic->insertIntoTable($this->cmsTable,$insertArr);
						if($ret)
						{
			...
			else
			{
				...
				if ($_FILES['cms_image']['name'] != "")
				{
					...
					else
					{
						...
						$ret   = $this->model_basic->updateIntoTable($this->cmsTable,$idArr, $updArr);
						if($ret)
						{
							$this->nsession->set_userdata('succmsg', "CMS updated successfully.");
						}
					...
					$ret   = $this->model_basic->updateIntoTable($this->cmsTable,$idArr, $updArr);
						...
						if(isset($ret))
						{
							$this->nsession->set_userdata('succmsg', "CMS updated successfully123.");
						}
						else
						{
