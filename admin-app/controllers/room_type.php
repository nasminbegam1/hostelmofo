...
class Room_type extends CI_Controller{
    ...
    public function __construct(){
		...
		function is_name_exists(){
				...
				$roomtype_name	= '';
				$room_type_name	= strip_tags(addslashes(trim($this->input->get_post('room_type_name'))));
		
				$whereArr	= array();
				if($id > 0){
						$whereArr	= array( 'roomtype_name' => $roomtype_name,'roomtype_id != ' => $id);
				}
				else{			
	...
	$this->form_validation->set_rules('bathroom_option', 'Bath Room Option', 'trim|required');
	$this->form_validation->set_rules('type_flag', 'Gender selection', 'trim|required');
	...
	if($this->input->get_post('action') == 'Process'){
		...
		{	
		    ...
		    $bathroom_option	= addslashes($this->input->post('bathroom_option'));
		    $number_of_bed	= trim($this->input->post('number_of_bed'));
			 $type_flag	= trim($this->input->post('type_flag'));
		    ...
		    $insertArr  =  array(
					    ...
					    'bathroom_option'		=> $bathroom_option
					);
	...
	$this->data['brdLink'][2]['name']   =   'Add New Room Type';
	...
	if($this->input->get_post('action') == 'Process'){			
	    ...
	    {                      
				...
				$roomtype_status	= addslashes($this->input->post('roomtype_status'));
				$room_price_type	= addslashes($this->input->post('room_price_type'));
				$bathroom_option	= $this->input->post('bathroom_option');
				$number_of_bed		= trim($this->input->post('number_of_bed'));
				$type_flag	= trim($this->input->post('type_flag'));		    
		    $insertArr  =  array(
														'roomtype_name'	    => $roomtype_name,
	...
	$this->data['brdLink'][2]['logo']   =   '';
	$this->data['brdLink'][2]['name']   =   'Edit Room Type';
	$this->data['brdLink'][2]['link']   =   'javascript:void(0)';
