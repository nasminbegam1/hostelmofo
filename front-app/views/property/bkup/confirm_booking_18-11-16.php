    ...
    <div class="bookFormPn globalClr popupInSec"> 
      <h3>Room Summary</h3>
		...
		if(isset($room_type_id) && is_array($room_type_id))
		{
				...
				//$total_price 					= $property_price*($total_day+1);
				...
				for($i = 0;$i<$total_day;$i++){
						...
						foreach($room_type_id as $k => $room){
								...
								$sql = "SELECT * FROM hw_availibility WHERE property_id = '".$property_id."' AND roomtype_id = '".$room_type_id[$k]."' AND DATE_FORMAT(date,'%Y-%m-%d') = '".$new_data1."'";
								
								$str ='SELECT  HAR.* FROM hw_agent_roomtype as HAR WHERE HAR.id = "'.$room_type_id[$k].'" AND HAR.property_id="'.$property_id.'"';
								$q = $this->db->query($str);
								$res = $q->result_array();
								//pr($res);
								...
								if($query->num_rows()>0){
										...
										$price = currentPrice1($record[0]['price'],$currencySymbol,$currencyRate);
								...
								$roomtypename   = $this->model_basic->getValue_condition('hw_agent_roomtype', 'type_name', '', 'id='.$room.'');
										if($k%2==0){
												$class  = "odd_class";	
										}
										else{
						...
						$room_price_per_day = ($res[0]['price_charge_type'] == 'per_person') ? $price1*$no_person[$k] : $price1;
						$total_room_price = $total_room_price + $room_price_per_day;
						
		?>
		
						<input type="hidden" name="room_price[<?php echo $k;?>]" value="<?php echo $price;?>">		
						...
						<input type="hidden" name="tot_room_price[<?php echo $k;?>]" value="<?php echo ($res[0]['price_charge_type'] == 'per_person') ? number_format($price1*$no_person[$k],2) : number_format($price1,2);?>">
						<?php
						...
						<tr class="<?php echo $class;?>">
							...
							<td><?php echo stripslashes(ucwords($roomtypename));?>
							
							</td>
							<td><?php //echo $price.','.$currencySymbol.','.$currencyRate;
							
							if($currencySymbol!=''){echo $currencySymbol; }else{ echo '$'; } ?> <?php echo round($price1);?></td>
							<td><?php echo $no_person[$k];?>
						  
							</td>
							<td><?php if($currencySymbol!=''){echo $currencySymbol; }else{ echo '$'; } ?> 
						<?php echo round($room_price_per_day);?></td>
						</tr>
