  ...
  <div class="allPopupBoxIn bigPop">
    <div class="bookFormPn globalClr popupInSec"> 
      <h3>Room Summary</h3>
		...
		<?php
		//pr($post_value,0);
			if(is_array($post_value['room_type_id'])){
				for($i=0;$i<count($post_value['room_type_id']);$i++)
				{
		?>
					<tr>
						<td><?php echo $post_value['room_name'][$i] ;?></td>
						<td><?php if($currencySymbol!=''){echo $currencySymbol; }else{ echo '$'; } ?><?php echo $post_value['room_price'][$i] ;?></td>
						<td><?php echo $post_value['no_room'][$i]." ".$post_value['hosteltype'][$i] ;?></td>
						<td><?php if($currencySymbol!=''){echo $currencySymbol; }else{ echo '$'; } ?><?php echo $post_value['tot_room_price'][$i] ;?></td>
					</tr>
					
					
					<input type="hidden" name="no_room[<?php echo $i;?>]" value="<?php echo $post_value['no_room'][$i];?>">
					<input type="hidden" name="room_type_id[<?php echo $i;?>]" value="<?php echo $post_value['room_type_id'][$i];?>">
					<input type="hidden" name="no_person[<?php echo $i;?>]" value="<?php echo $post_value['no_room'][$i]?>">
					<input type="hidden" name="room_price[<?php echo $i;?>]" value="<?php echo $post_value['room_price'][$i];?>">
					<input type="hidden" name="room_price[<?php echo $i;?>]" value="<?php echo $post_value['room_price'][$i];?>">
					<input type="hidden" name="tot_room_price[<?php echo $i;?>]" value="<?php echo $post_value['tot_room_price'][$i];?>">
					<input type="hidden" name="hosteltype[<?php echo $i;?>]" value="<?php echo $post_value['hosteltype'][$i];?>">
					<input type="hidden" name="room_name[<?php echo $i;?>]" value="<?php echo $post_value['room_name'][$i];?>">
		...
		$total_room_price 		= $post_value['property_price_total'];
		...
		<table width="80%" cellspacing="0" cellpadding="0" border="0" class="paymentsummary">             
				<tr>
                <td width="80%" class="paynow" align="right">Total: <?php //echo ($total_price.','.$currencySymbol.','.$currencyRate);?></td>
		...
		<tr>
                ...
                <td class="paynow" align="left"><span><?php if($currencySymbol!=''){echo $currencySymbol; }else{ echo '$'; } ?></span>
						<span><?php echo $deposit_amount;?></span>
		<input type="hidden" name="deposit_amt" id="deposit_amt" value="<?php echo $deposit_amount;?>">
		...
		<tr id="standard_flexible_amount_row" style="display:none;">
				<td width="80%" class="standard_flex_now" align="right"><?echo $standard_flexible_amount;?>% Deposit/Downpayment for Standard Flexible Booking:</td>
				<td align="left"><span><?php if($currencySymbol!=''){echo $currencySymbol; }else{ echo '$'; } ?> <span><?php echo $flexible_deposite_amount;?></span></td>
 ...
 <div class="targetDiv" id="discount_code_section" style="display: block;" >
  ...
  <div class="allPopupBoxIn bigPop">
	...
	<div class="innr_two_col">
	   ...
	   <input placeholder="Phone Number*" onkeydown="javascript:return Numeric(event);" name="suffix_phone" id="suffix_phone" type="text" style="width: 422px;">
	  </div>
	<div class="innr_two_col">  
...
<input type="hidden" id="property_price_total" name="property_price_total"  value="<?php echo $total_room_price;?>">
