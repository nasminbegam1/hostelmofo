...
-->
    <div class="globalClr" data-scroll-index='4'>
	 ...
	 <div id="room_not_available" style="display: none; height: 50px; color: red;"> This location have no rooms </div>
        ...
        <div class="detailsBlock globalClr bookTable priceTable" style="display: none;">
	    ...
	    if(count($details['price'])>0){
	       $j = 0;
			 ...
			 //if(($data['room_price_type'] == 'per_night' && !($data['no_of_booked'] >= $data['no_of_rooms'])) || ($data['room_price_type'] == 'per_person' && !($data['no_of_booked'] >= $data['size']))){
			// if( ($data['price_charge_type'] == 'per_night' && ($data['no_of_rooms'] > $data['no_of_booked']) ) || ($data['price_charge_type'] == 'per_person' && ($data['size'] > $data['no_of_booked']) )){
			?>
            <tr>
              <td align="left" valign="top"><div class="tool"><a href="javascript:void(0)" class="tooltrip_show"><span class="tooltrip"><?php echo stripslashes(ucwords($data['room_description'])); ?></span>
				  <?php echo stripslashes(ucwords($data['type_name'])); ?></a></div><span style="color:blue;display: block;" class="avl_room_<?php echo $j;?>"></span></td>
              <td align="left" valign="top"><table class="rateTable"  width="100%" border="0" cellspacing="0" cellpadding="0">
                  <tr>
                    <?php for($i=1;$i<=8;$i++){?>
