			...
			<div class="cmsBlock globalClr">
				<h1 class="globalClr"><?php echo stripslashes($province_details['province_name']);?></h1>
				<p><?php echo stripslashes($province_details['province_description']);?></p>
			</div>
			
			<div class="favcity globalClr clearfix">
				<?php //pr($fav_location_arry);
				if(is_array($city_under_province) && count($city_under_province) > 0)
				{
				?>	
					<ul>
				<?php		
					foreach($city_under_province as $v)
					{
						echo "<li><a href='".ORIGINAL_SITE_URL.$province_slug."/".$v['city_seo_slug']."/'>".stripcslashes($v['city_name'])."</a></li>";
					}
				?>	
					</ul>
				<?php	
			...
			<div class="searchContainer globalClr cmsSearch">
				<div class="searchPan globalClr active mainSearchPanel">
					<div class="searchPanIn globalClr clearfix">
						...
						<div class="searchBox1 searchBox1Two searchBoxThree ltCls">
