...
class Property extends MY_Controller{
	...
	public function details(){ 
		...
		if($city!=''){
			...
			$breadcrumb_arr[]	= array('text'=>$province_arr['province_name'],'link'=>'');
		...
		//$this->data['chkindt'] = $chkindt[2]."-".$chkindt[0]."-".$chkindt[1];
		//$this->data['chkoutdt'] = $chkoutdt[2]."-".$chkoutdt[0]."-".$chkoutdt[1];
		...
		$this->data['checkin_date'] = (isset($this->data['check_in']) && $this->data['check_in']) ? $this->data['check_in'] : date('D jS M Y',strtotime(str_replace("/", "-", DEFAULT_CHECK_IN_DATE))) ;
		
		$this->data['checkout_date']=  (isset($this->data['chkoutdt']) && $this->data['chkoutdt'])? $this->data['chkoutdt'] : date('D jS M Y',strtotime(str_replace("/", "-", DEFAULT_CHECK_OUT_DATE))) ;
		...
		//echo "<><><>".$this->data['chkindt'];
		//echo "<><><>".$this->data['chkoutdt'];
		
		$arr_check_in 	= explode("-", $this->data['check_in']);
		$check_in_date	= $arr_check_in[0]."-".$arr_check_in[2]."-".$arr_check_in[1];
		
		$arr_check_out	= explode("-", $this->data['check_out']);
		$check_out_date	= $arr_check_out[0]."-".$arr_check_out[2]."-".$arr_check_out[1];
		...
		//echo "<><>".$this->data['check_in'];
		//echo "<><>".$this->data['check_out'];
		
		$arr1_check_in 	= explode("-", $this->data['check_in']);
		$check_in_date	= $arr1_check_in[1]."/".$arr1_check_in[2]."/".$arr1_check_in[0];
		
		$arr1_check_out	= explode("-", $this->data['check_out']);
		$check_out_date	= $arr1_check_out[1]."/".$arr1_check_out[2]."/".$arr1_check_out[0];
		
		$this->data['check_in'] = $check_in_date;
		$this->data['check_out'] = $check_out_date;
		
		$arr_data_chkin = explode("-", $this->data['chkindt']);
		$this->data['chkindt'] = $arr_data_chkin[0].'-'.$arr_data_chkin[2].'-'.$arr_data_chkin[1];
		
		
		$this->templatelayout->make_seo($title,$desc,$key,$share);
		//$this->templatelayout->get_header();
		
		$this->templatelayout->get_banner($city_slug,$location,$property_type_slug,$property_type,$property_slug,$check_in_date,$check_out_date,$this->data['guest'],$group_type,$age_ranges,$breadcrumb_arr);
		...
		$this->layout->multiple_view($this->elements,$this->elements_data);
		  ...
		  if($this->uri->total_segments()>12){
				...
				$this->templatelayout->get_banner_inner('',$property_slug);
				//$this->templatelayout->get_banner($city_slug,$location,$property_type_slug,$property_type,$property_slug,$this->data['check_in'],$this->data['check_out'],$this->data['guest'],$group_type,$age_ranges,$breadcrumb_arr);
				$this->templatelayout->get_breadcrumb($breadcrumb_arr);
				$this->templatelayout->get_inner_footer();
		  ...
		  public function confirmbooking(){
			...
			{
					$user 			= $this->model_basic->getValues_conditions(USER,'','',"id=".$this->nsession->userdata('USER_ID'));
					$prefix_phone 	= $user[0]['country_code'];
					$suffix_phone 	= $user[0]['mobile_no'];
					
					$cardInfo =	$this->model_basic->getValues_conditions(USER_CREDIT_CARD,"","","country_code ='".$prefix_phone."' AND phone_number ='".$suffix_phone."'");
					
					$this->data['card_info'] = $cardInfo;
			}
			...
			if($record_exist_bookingtemp == 0){ 
				 ...
				 if($this->input->post('bookingType') == 'booking'){
					  ...
					  {
							  $ci = explode('-',$check_in);
							  $ci_date = $ci[0]."-".$ci[2]."-".$ci[1];
							  $co = explode('-',$check_out);
							  $co_date = $co[0]."-".$co[2]."-".$co[1]; 
					  }
					  
					 
					  //echo $check_in."===".$ci_date; die();
		  ...
		  $data['first_name'] 	= $dataArr['first_name'];
		  $data['last_name'] 	= $dataArr['last_name'];
		  $data['email'] 		= $dataArr['email'];
		  $data['gender']	    = $dataArr['gender'];
		  $data['nationality'] 	= $dataArr['nationality'];
		  $data['country_code'] = $prefix_phone;
		  $data['phone_number'] = $suffix_phone;
		  $data['card_name'] 	= $this->input->post('cc_holder_name');
		  $data['card_number']	= $cc_number;
		  
		//  if($exp_date != '')
		//  {
		//		$new_exp = explode("/",$exp_date);
		//		if(count($new_exp)>1)
		//		{
		//			$data['expiry_date']	= "20".$new_exp[1]."-".$new_exp[0].'-01';
		//		}
		//		
		//			
		//  }
		  ...
		  $data['expiry_date']    = "20".$exp_year."-".$exp_month."-01";
		  
		
					  
			  $isExists =	$this->model_basic->isRecordExist(USER_CREDIT_CARD,"card_number ='".$data['card_number']."' AND country_code ='".$prefix_phone."' AND phone_number ='".$suffix_phone."'");
			  if($isExists == 0 && $cc_number >0 && $data['card_name'] != '')
			  {
				 $this->model_basic->insertIntoTable(USER_CREDIT_CARD,$data);	
				
			  }
			  
				$chkin 	= explode('-',$dataArr['check_in']);
				$chkout = explode('-',$dataArr['check_out']);
				...
				//echo "bbb"; pr($dataArr);
					...
					if(!empty($ins_id) && $ins_id >0){
						...
						if(count($payment)>0){
							...
							$this->nsession->set_userdata('payment_id',$payment_id);
						
							foreach($bookingIds as $ids){
								unset($ids['booking_id']);
								$ids['payment_id'] = $payment_id;
					...
					$exp_date = $exp_month."/".$exp_year;
					$dataArr['card_expiry_date']    = "20".$exp_year."-".$exp_month."-01"; 
					//if($exp_date != '')
					//{
   ...
   public function success_booking(){
    ...
    public function sendEnqueryMessage(){
       ...
       $message  = $this->load->view('email_template/enquiry_template',
					   ...
					   //'checkin_date' => date('m/d/Y',strtotime($check_in_time)),
                                           //'checkout_date' => date('m/d/Y',strtotime($check_out_time)),
                                           'content'=>$notes),true);
