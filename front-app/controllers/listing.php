	...
	public function __construct(){
		...
		public function index(){
				...
				if($check_in == ''){
					...
					if($check_out == ''){
								$check_out = date('d/m/Y',strtotime(date("Y-m-d") . " +3 days"));
					}
				//echo $check_out;
				if(isset($property) && $property != ''){
					...
					if(isset($check_out) && $check_out != ''){
						...
						$this->data['map_url']			= '?guest='.$guest.'&type='.$ptype.'&province='.$province.'&city='.$city.'&checkin='.$chec_in.'&checkout='.$chec_out.$extra_para.'&typeid='.$type_id."&s=true";
						...
						$this->templatelayout->get_banner_listing($city, $temploc, $ptype, $type_id,$property, $baner_check_in, $baner_check_out,$guest,$group_type,$age_ranges,$breadcrumb_arr);
