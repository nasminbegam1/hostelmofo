...
class Property extends MY_Controller{
		  ...
		  public function details(){
				...
				$featured_image='';
				if(is_array($this->data['details']['featured_img']) and array_key_exists('image_name',$this->data['details']['featured_img'])){ 						$featured_image =  isFileExist(CDN_PROPERTY_BIG_IMG.$this->data['details']['featured_img']['image_name']);
				}
				$share = array(
                            'og'=>array(
                                        'site_name'=>$title,
					...
					// exit;
					 ...
					 if($new_user_id != ''){
						  ...
						  if($nationality == '' && $gender == ''){
								...
								$this->model_basic->updateIntoTable(USER,
																				array('id'=>$new_user_id),
																				array('nationality'=>$nationality,'gender'=>$gender));
						  }
					 }
		...
		/*******correct upto***************/
					 ...
					 $balanceAmount = $property_price - $payble_amount;
					 ...
					 if($wallet_balance > $payble_amount || $wallet_balance == $payble_amount){
								...
								if(!empty($ins_id) && $ins_id >0){
										  ...
										  $this->model_basic->updateIntoTable('hw_payment_info_temp',
																				array('paymeny_id'=>$ins_id),
																				array('reference_id'=>getInsID($ins_id))
										  							  );
										...
										if(isset($room_type_id)){
										  $message = '<table cellspacing="0" cellspadding="0" style="border : 0px;" width="100%"><tr><td style="border-bottom :1px solid #ccc;border-right :1px solid #ccc;padding: 5px 10px; font-weight: bold;">Room types chosen</td><td style="border-bottom :1px solid #ccc;border-right :1px solid #ccc;padding: 5px 10px; font-weight: bold;">No. Rooms</td><td style="border-bottom :1px solid #ccc;border-right :1px solid #ccc;padding: 5px 10px; font-weight: bold;">No. Guests</td><td style="border-bottom :1px solid #ccc;padding: 5px 10px; font-weight: bold;">Total Price</td></tr>';
										  ...
										  for($i=0;$i<count($room_type_id);$i++){
													 ...
													 $roomtypename   = $this->model_basic->getValue_condition('hw_agent_roomtype',
																																'type_name',
																																'',
																																'id='.$insertArr['room_type'].'');
													 $message .= "<tr>
										  ...
										  $message .= '<tr><td colspan="3" style="border-right :1px solid #ccc;padding: 5px 10px;"><b>Total Price (Rounded off)</b></td><td style="padding: 5px 10px;"><b>$ <span>'.round($property_price).'</span></b></td></tr></table>';
										}
								...
								if(count($payment)>0){
										  ...
										  if(isset($payment_id) && isset($details_id)){
													 ...
													 $property_name              = $this->model_basic
																												->getValue_condition('hw_property_master',
																																			'property_name', '',
								...
								if($payble_amount>0){
										  ...
										  //end of Avalibility table update		  
													 ...
													 $message .= '<tr><td colspan="3" style="border-right :1px solid #ccc;padding: 5px 10px;"><b>Total Price (Rounded off)</b></td><td style="padding: 5px 10px;"><b>$ <span>'.round($property_price).'</span></b></td></tr></table>';
																...
																$payment = $this->model_basic->getValues_conditions('hw_payment_info_temp','','','paymeny_id='.$ins_id);	  																if(count($payment)>0){
																		  $wallet = $this->model_basic->insertIntoTable(WALLET,$insrtArr);
				    ...
				    $this->data['check_in'] = $this->input->post('checkindt');
				    $this->data['check_out'] = $this->input->post('checkoutdt');
	...
	//pr($this->data);
					...
					//$this->templatelayout->get_banner_inner('','hostel');
					$this->templatelayout->get_banner($city_slug,$location,$property_type_slug,$property_type,$property_slug,$this->data['check_in'],$this->data['check_out'],$this->data['guest'],$group_type,$age_ranges,$breadcrumb_arr);
				    $this->templatelayout->get_breadcrumb($breadcrumb_arr);
   ...
   public function success_booking(){
    ...
    public function setApprove(){
		  ...
		  public function getDealRoomType(){
				$deal_id = $this->input->post('deal_id');
				$room_type = $this->model_property->getDealRoomType($deal_id);
