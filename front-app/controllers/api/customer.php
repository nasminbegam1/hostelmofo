...
class Customer extends REST_Controller{
				...
				if(!empty($data['first_name']) && !empty($data['last_name']) && !empty($data['email']) && !empty($data['dob_dd']) && !empty($data['dob_mm']) && !empty($data['dob_yy']) && !empty($data['nationality']) && !empty($data['country_code']) && !empty($data['mobile_no']) && !empty($data['password'])){
						...
						if(!$chkEmailCnt){
								...
								if($insertedID <> '')
								{
									...
									$this->model_basic->insertIntoTable('hw_app_user_tokens', $tokenArr);
									
								    $userArr = $this->model_basic->getSingleRow(USER, '',  '', 'id='.$insertedID);
								    ...
								    $message = array('data' => $userArr, 'status' => '1', 'message' => 'Your account has been created successfully.');
								}
								else{
										$message = array('status' => '2', 'message' => 'Unable to process. Please try again!');
								}
						}
						else{
	...
	//$customerRecord				= $this->model_basic->getValues_conditions('hw_app_user_tokens', '*', '', 'users_id = "'.$data['id'].'"');
