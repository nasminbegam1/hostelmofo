...
class Master extends REST_Controller{
	...
	$finalArr['allBooking'] 	= $this->model_api->all_booking($user_id);
    ...
    public function setCreditCard_post(){
		...
		if($data['card_number'] == '')
		{
			$message = array('status' => '0', 'message' => 'Card number cannot be blank.');
			$isblank = '1';
		}
		if($data['country_code'] == '')
		{
			$message = array('status' => '0', 'message' => 'Country code cannot be blank.');
			$isblank = '1';
		}
		if($data['phone_number'] == '')
		{
			$message = array('status' => '0', 'message' => 'Phone number cannot be blank.');
			$isblank = '1';
		}
		...
		if($isblank == 0)
		{
			...
			$isExists =	$this->model_basic->isRecordExist(USER_CREDIT_CARD,"card_number ='".$data['card_number']."'");
			if($isExists > 0)
			{
				$message = array('status' => '0', 'message' => 'This card already saved.');	
