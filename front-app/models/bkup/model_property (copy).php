                ...
                if(isset($pp_result) && is_array($pp_result) && count($pp_result)>0){
                    ...
                    foreach($pp_result as $res){
                        ...
                        if($room_price_type == 'per_night'){
                            //$room_price_type[$j]  = $room_price_type;
                            //$size[$j][] = $result_room_type_exists[0]['size'];
                           $sql =   "SELECT SUM(HBD.no_of_room) as total_room_booked FROM hw_payment_info as HPI INNER JOIN hw_booking_deatils AS HBD ON HBD.payment_id = HPI.paymeny_id AND HBD.room_type = ".$room_type." WHERE '".$chkdate."'  between HPI.check_in AND DATE_ADD(HPI.check_out, INTERVAL -1 DAY) AND HPI.property_id = ".$property_id." AND Booking_status = 'Booked' ";
                            ...
                            //$room_price_type[$j][]  = $room_price_type;
                           $sql =   "SELECT SUM(HBD.no_of_person) as total_person_booked FROM hw_payment_info as HPI INNER JOIN hw_booking_deatils AS HBD ON HBD.payment_id = HPI.paymeny_id AND HBD.room_type = ".$room_type."  WHERE '".$chkdate."'  between HPI.check_in AND DATE_ADD(HPI.check_out, INTERVAL -1 DAY) AND HPI.property_id = ".$property_id." AND Booking_status = 'Booked' ";
                            $query          =   $this->db->query($sql);
                            $available_arr  =   $query->result_array();
                    ...
                    /****************
                        ...
                        $room_type_exists =   "SELECT HRM.room_price_type,HRM.number_of_bed,HAR.size,HAR.no_of_rooms FROM hw_agent_roomtype AS HAR INNER JOIN hw_roomtype_master as HRM ON HRM.roomtype_id = HAR.room_type_master_id AND HRM.roomtype_status = 'Active' WHERE HAR.room_type_master_id = ".$room_type_master_id;
                        //echo $room_type_exists; exit;
                    $room_type_exists_res = $this->db->query($room_type_exists);
                    $result_room_type_exists = $room_type_exists_res->result_array();                                                                                             
                    if($result_room_type_exists[0]['room_price_type'] == 'per_night')
                    {
                        $room_price_type[$j][]  = $result_room_type_exists[0]['room_price_type'];
                        $size[$j][] = $result_room_type_exists[0]['size'];
                        $sql            =   "SELECT SUM(HBD.no_of_room) as total_room_booked
                        FROM hw_payment_info as HPI
                        INNER JOIN hw_booking_deatils AS HBD ON HBD.payment_id = HPI.paymeny_id AND HBD.room_type = ".$room_type."
                      ...
                      /***************  
                        ...
                        $available_arr  =   $query->result_array();
                         ...
                         if($available_arr[0]['total_room_booked'] >= $result_room_type_exists[0]['no_of_rooms']  && $available_arr[0]['total_room_booked'] != '')
                        {
                            $available[$j][]  = 0;
                        }
                        else
                        {
                            $avl_no_room = $result_room_type_exists[0]['no_of_rooms'] - $available_arr[0]['total_room_booked'];
                            $available[$j][] = $avl_no_room * $result_room_type_exists[0]['size'];
                        }
                    }
                    else
                    {
                        ...
                        $no_of_bed = $result_room_type_exists[0]['no_of_rooms'] * $result_room_type_exists[0]['size'];
                        $query          =   $this->db->query($sql);
                        $available_arr  =   $query->result_array();
