    ...
    public function getFavouriteDetails($uid,$currency=''){
        ...
        if($query->num_rows()>0){
            ...
            for($k=0; $k<count($record['master_details']); $k++){
                ...
                PP.isDefault,
                PP.updated_on,
                RM.roomtype_name,RM.room_price_type  FROM ".PROPERTY_ROOMPRICE." AS PP JOIN ".ROOMTYPE_MASTER." AS RM ON PP.room_type= RM.roomtype_id WHERE PP.property_id='".$property_id."' AND isDefault= 'Yes' order by number_of_bed desc LIMIT 1";
