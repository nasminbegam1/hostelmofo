...
class Model_api extends CI_Model{
    ...
    public function getFavouriteDetails($uid,$currency=''){
        ...
        $str ='SELECT (SELECT count(*) FROM hw_feedback as HF where HF.property_id = PM.property_master_id) AS feedback_count, PM.property_master_id,PM.property_name,PM.property_slug,PM.property_type_id,PM.bedrooms,PM.beds,PM.room_type,PM.facilities,PD.address,PD.address2,PT.property_type_name,PD.province_id,PD.zip_code,PD.city_id,PD.brief_introduction,PD.latitude,PD.longitude,PC.city_name,PC.city_seo_slug,PP.province_name,PP.province_slug,PT.property_type_name,PT.property_type_slug
        ...
        if($query->num_rows()>0){
            ...
            for($k=0; $k<count($record['master_details']); $k++){
                ...
                $record['master_details'][$k]['featured_img']= new stdClass();
    ...
    public function propertygetlist($searchFrom = 'search',$currency='', $lcity = '', $lprovince='', $lproptype='',$logged_id = ''){
        ...
        //        AND PD.latitude != '' AND PD.longitude != '' 
        //        ".$where."                
        ...
        //        ".$sort_sql." ";
                ...
                $sql = "SELECT (SELECT count(*) FROM hw_feedback as HF where HF.property_id = PM.property_master_id) AS feedback_count, ROUND(((SUM(F.value_for_money)+ SUM(F.security)+ SUM(F.location)+ SUM(F.staff)+ SUM(F.atmosphere)+ SUM(F.cleanliness)+ SUM(F.facilities))/(COUNT(F.feedback_id)*5*7))*100) totalFeedback,PM.property_master_id as property_id, PM.property_name, PM.property_slug, PM.property_type_id, PM.property_master_id, PM.bedrooms, PM.beds, PM.room_type, PM.facilities,PD.address, PD.address2, PD.province_id, PD.zip_code, PD.city_id, PD.brief_introduction, PD.latitude, PD.longitude,CITY.city_name, CITY.city_seo_slug, (SELECT image_name FROM ".PROPERTY_IMAGE." AS IM WHERE IM.property_id = PM.property_master_id AND IM.featured_image = 'Yes') AS image_name, PR.province_name, PR.province_slug,PT.property_type_name, PT.property_type_slug 
                FROM ".PROPERTY_MASTER." PM  
                INNER JOIN ".PROPERTY_DETAILS." PD ON PM.property_master_id = PD.property_id
                INNER JOIN ".PROPERTY_TYPE." PT ON PM.property_type_id = PT.property_type_id 
                INNER JOIN ".CITY." CITY ON PD.city_id = CITY.city_master_id
              ...
              //echo $sql;  
                ...
                //$sql = "SELECT  COUNT(F.feedback_id) AS feedback_count,    ROUND(((SUM(F.value_for_money)+SUM(F.security)+SUM(F.location)+SUM(F.staff)+SUM(F.atmosphere)+SUM(F.cleanliness)+SUM(F.facilities))/(COUNT(F.feedback_id)*5*7))*100) totalFeedback,
                //
       ...
       // exit;
        ...
        if($rec && is_array($rec) ){
            ...
            foreach($rec as $r){
                ...
                if($dorm_query->num_rows() > 0){
                    ...
                    $dprice = min( $dorm_res );                  
                    $rec[$i]['min_dorm'] = $this->convertPrice($dprice['base_price'],$currencyRate);
                }
                else{
                    $rec[$i]['min_dorm'] = 0;
                }
                ...
                if($private_query->num_rows() > 0){
                    ...
                    $dprice = min( $private_res ); 
                    $rec[$i]['min_private'] = $this->convertPrice($dprice['base_price'],$currencyRate);
                }
                else{
                    $rec[$i]['min_private'] = 0;
                }
                ...
                if($min_qr->num_rows() > 0){
                    ...
                    $dprice = min( $min_price ); 
                    $rec[$i]['min_price'] = $this->convertPrice($dprice['base_price'],$currencyRate);
                }
                else{
                    $rec[$i]['min_price'] = 0;
                }
        ...
        if(is_array($rec))
        {
            foreach($rec as $k=>$r)
            {
                //$imagepath = isFileExist(CDN_PROPERTY_SMALL_IMG.stripslashes($r['image_name']));
                
                //if($imagepath == '')
                //{
                //    $r['image_name'] = "no_img.jpg";
                //}
                //else
                //{
                //    $r['image_name'] = $imagepath;
                //}
                
                $file_path = CDN_PROPERTY_SMALL_IMG.stripslashes($r['image_name']);
                ...
                if($r['daily_price']>0)
                {
                    if($r['min_private'] >0 )
                    {
                        $r['daily_price'] = $r['min_private'];
                    }
                    if($r['min_dorm'] >0 )
                    {
                        $r['daily_price'] = $r['min_dorm'];
                    }
                    $new_rec[] = $r;
                }
        ...
        if($property_slug!=''){
            ...
            if($query->num_rows()>0){
                ...
                if($i_query->num_rows()>0){
                    ...
                    foreach($i_query->result_array() as $k=>$img){
                        
                        $file_path = CDN_PROPERTY_BIG_IMG.stripslashes($img['image_name']);
                        $ch	= curl_init($file_path);
                        curl_setopt($ch, CURLOPT_NOBODY, true);
                        curl_exec($ch);
                        ...
                        if($retcode == 200)
                        {
                            $record['images'][$k]['image_name'] =  $img['image_name'];
                            $is_image_exists = 'Yes';
                        }
                        else
                        {
                           $record['images'][$k]['image_name']  =  'no_img.jpg';
                           $is_image_exists = 'No';
                            
                        }
                       
                        
                        if($img['featured_image']=='Yes' && $is_image_exists == 'Yes'){
                            $record['featured_img'] = $img;
                        }
                    }
                }
                else{
                    $record['images'][0]['image_name']  =  'no_img.jpg';
                }
    ...
    public function all_booking($user_id='')
    {	
        $sql = "SELECT PM.property_name,PI.property_id,PI.paymeny_id,         PI.added_date,PI.check_in,PI.check_out,UM.id,PI.Booking_status,PI.paymeny_id,HCM.city_name,HPI.image_name,HPI.image_title,HPI.image_name
    ...
    {	
        ...
        INNER JOIN hw_property_image as HPI ON HPI.property_id = PM.property_master_id AND HPI.featured_image = 'Yes'
        INNER JOIN ".USER." AS UM ON PI.user_id = UM.id
        WHERE PI.property_id not in (select HF1.property_id from hw_feedback as HF1 WHERE HF1.user_id = ".$user_id." AND HF1.property_id = PI.property_id)  AND PI.check_out <= NOW()- INTERVAL 1 DAY AND PI.user_id= ".$user_id." AND PI.Booking_status='".$book."' GROUP BY PI.paymeny_id";
...
//				}
    ...
    public function convertPrice($price='',$rate=''){
