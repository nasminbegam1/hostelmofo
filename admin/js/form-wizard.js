$(function () {
    ...
    $('#rootwizard-custom-circle').bootstrapWizard({
        ...
        'onTabShow': function(tab, navigation, index) {
            ...
            if ($("#nextAddProperty").hasClass('disabled')==true) {
                $("#nextAddProperty").html('Save<i class="fa fa-wrench mlx"></i>');
            }else{
                $("#nextAddProperty").html('Next<i class="fa fa-arrow-circle-o-right mlx"></i>');
            }
            $("#nextAddProperty").css("pointer-events",'auto');
            $("#nextAddProperty").css("cursor",'pointer');
            $("#nextAddProperty").css("o",'pointer');
        },
        'onNext': function(tab, navigation, index) {
    ...
    $(".editRrmBtn").click(function(){
           var $approved = 1;
            // Check all input validation
            $(".PropEditForm .requiredInput").each(function(){
    ...
    $(".addPropBtn").click(function(){
           var $approved = 1;
            // Check all input validation
            $("#propertyAddFrm .requiredInput").each(function(){
                 if (!$(this).val()) {
                     $(this).css('border-color', 'red');
                     $(this).parent().parent().find("i.alert").removeClass("alert-hide");
                     $approved = 0;
            ...
            if ($("#propertyAddFrm .propRomeType").length >0) {
                if($("#propertyAddFrm .propRomeType:checked").length == 0){
                   $('.rType').removeClass("alert-hide");
                    $approved = 0;
                  }
                  else
                  {
                    
                    $('.rType').addClass("alert-hide");
...
function formValidate($id){
      var $approved = 1;
            // Check all input validation
        $($id + " .requiredInput").each(function(){
             if (!$(this).val()) {
                 $(this).css('border-color', 'red');
                 $(this).parent().parent().find("i.alert").removeClass("alert-hide");
                 $approved = 0;
        ...
        if ($($id + " .propRomeType").length >0) {
            if($($id + " .propRomeType:checked").length == 0){
