...
add_action( 'wp_ajax_nwletter', 'submit_newsletter' );
add_action( 'wp_ajax_nopriv_nwletter', 'submit_newsletter' );
...
function submit_newsletter() {
    ...
    $output = '';
    $id = '';
    $nw_name 	= addslashes(trim($_POST['nw_name']));
    $nw_email 	= addslashes(trim($_POST['nw_email']));
    $wpdb->get_results("SELECT * FROM hw_newsletter WHERE email='".$nw_email."'");
    $exist      = $wpdb->num_rows;
    if($exist == 0){
        $insertArr = array('first_name' => $nw_name,'email'=> $nw_email);          
        $wpdb->insert( 'hw_newsletter', $insertArr, $format );
        $id = $wpdb->insert_id;
        if($id){
            $output =  'success||Newsletter subscribed successfully!';
        }else{
            $output =  'error||Unable to subscribe newsletter';
        }
    }else{
        $output = 'error||You have already subscribed for newsletter';
    } 
    die($output);
}
