...
class Property extends MY_Controller{
	...
	$this->data['booking_link']	= AGENT_URL."propertybooking/bookings/{{ID}}/";
...
//   }
   ...
   public function addAction(){
	  ...
	  $this->form_validation->set_rules('room_type[]','Room Type','required|trim');
	    ...
	    if($this->form_validation->run() == true){
	       ...
	       //$property_facilities_str	= addslashes( implode(',',$this->input->post('property_facilities')) );
   ...
   public function edit($property_id){
	...
	$brd_arr[] = array('link'=>base_url('property'), 'text' => 'Property','icon_class'=>'icon-briefcase' );
	$brd_arr[] = array('link'=>'javascript:void(0);', 'text' => 'Edit' );
	...
	$this->nsession->set_userdata('succmsg', "");		
	$this->nsession->set_userdata('errmsg', "");
        $this->data['tabs'] = $this->load->view('property_tab',array('select_tab'=>'bookings'),true);
   ...
   public function editAction(){
       ...
       if($this->input->get_post('action') == 'basic_info')
       {
	    ...
	    //$this->form_validation->set_rules('room_type','Room_type','required|trim');
	    if($this->form_validation->run() == true)
	    {
		...
		redirect(AGENT_URL.'property/editcontactaction/'.$property_id.'/'.$page.'/');
		...
		//echo validation_errors();
	        ...
	        redirect(AGENT_URL.'property/edit/'.$property_id.'/'.$page.'/');
	    }
       }
	...
	if($this->input->get_post('action') == 'basic')
       {
		...
		redirect(AGENT_URL.'property/editpriceaction/'.$property_id.'/'.$page.'/');
	    }else{
		$this->nsession->set_userdata('errmsg', preg_replace('/\s+/', ' ',validation_errors('<p>','</p>')));
		redirect(AGENT_URL.'property/editbasicaction/'.$property_id.'/'.$page.'/');
	    }
   ...
   public function editpriceaction()
   {
	...
	$this->model_basic->updateIntoTable(PROPERTY_MASTER, $idArr, $updateArr);
	    ...
	    redirect(AGENT_URL.'property/editfacilitiesaction/'.$property_id.'/'.$page.'/');
	...
	$this->data['policy_list'] 		= $this->model_basic->getValues_conditions(POLICY_MASTER,'','','status="Active"');
	 ...
	 if($this->input->get_post('action') == 'facilities')
		...
		$this->nsession->set_userdata('succmsg','Property facilities & policy details is updated successfully');
	        redirect(AGENT_URL.'property/editimageaction/'.$property_id.'/'.$page.'/');	
	}
   ...
   public function editimageaction()
   {
	...
	if($this->input->get_post('action') == 'photo')
		...
		$this->nsession->set_userdata('succmsg','Property photo & video details is updated successfully');
		//echo AGENT_URL.'property/index';exit;
		redirect(AGENT_URL.'property/index/');
		//exit;
   ...
   public function editcontract()
   {
	...
	$this->data['property_details'] = $this->model_property->getPropertyDetails($property_id);
	$this->data['contract_info'] = $this->model_basic->getValue_condition(CMS,'cms_content','',' cms_id=46');
	
	$this->data['breadcrumbs'] = $brd_arr;
	
	if($this->input->get_post('action') == 'contract')
        {
		$contract_term = $this->input->post('contract_term');
	...
	$this->nsession->set_userdata('succmsg', "");		
	$this->nsession->set_userdata('errmsg', "");
        $this->data['tabs'] = $this->load->view('property_tab',array('select_tab'=>'contract'),true);
        $this->templatelayout->get_header();
	$this->templatelayout->get_footer();
	$this->templatelayout->get_sidebar('property');
        $this->elements['middle']='property/edit_contract';			
        $this->elements_data['middle'] = $this->data;			    
        $this->layout->setLayout('layout');
        $this->layout->multiple_view($this->elements,$this->elements_data);
   }
